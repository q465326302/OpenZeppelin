...
createNotificationChannel函数需要CreateNotificationRequest对象，并返回NotificationResponse对象。
```
...
```
一旦所有必需的参数填充完毕，您可以通过在客户端上调用 create 函数来创建一个 Sentinel。这将返回一个 CreateSentinelResponse 对象。
```

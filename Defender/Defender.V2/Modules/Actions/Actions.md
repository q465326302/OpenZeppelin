# Actions
动作允许你实现自定义应用逻辑，用于链上和链下操作。你可以启用自动响应，以应对*监控*和*事件响应*模块检测到的威胁。
...
* 在Defender 2.0监控器的情况下，正文将包含*监控器事件模式*。

* 在Forta监控器的情况下，正文将包含*Forta警报详情*。
...
* 将package.json中的dependencies键设置为上面*环境部分*中指示的包
...
### Approval Process
定义你希望如何执行交易。你可以从Defender 2.0中可用的任何*交易审批流程*中选择，这些流程你之前已经配置过，或者你可以选择创建一个新的。
...
> NOTE
我们提供了一个快速入门教程，用于使用 Defender 2.0 为智能合约创建自动操作。在*这里*查看吧！
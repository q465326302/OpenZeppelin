...
## 标准接口
[IAccount.cairo](https://github.com/OpenZeppelin/cairo-contracts/blob/ad399728e6fcd5956a4ed347fb5e8ee731d37ec4/src/openzeppelin/account/IAccount.cairo)合约接口包含了在[#41](https://github.com/OpenZeppelin/cairo-contracts/discussions/41)中提出并被OpenZeppelin和Argent采用的标准账户接口。它实现了[EIP-1271](https://eips.ethereum.org/EIPS/eip-1271)，并且对签名验证和nonce管理策略是不可知的。
...
### MockSigner实用程序
utils.py中的MockSigner类用于在给定的账户上执行交易，构建交易并管理nonce。

交易的流程始于检查nonce并将每个调用的合约地址转换为十六进制格式。十六进制转换是必要的，因为Nile的Signer将地址转换为基于16的整数（需要一个字符串参数）。请注意，直接转换为字符串最终会导致整数超过Cairo的FIELD_PRIME。

交易中包含的值被传递给Nile的Signer的sign_transaction方法，该方法创建并返回一个签名。最后，MockSigner实例调用账户合约的__execute__方法并传递交易数据。
...
```
如果您想要在AccountRegistry合约上更新账户的L1地址，您可以进行以下操作。
```
await signer.send_transaction(account, registry.contract_address, 'set_L1_address', [NEW_ADDRESS])
```
您可以阅读有关[账户消息方案讨论中消息结构和哈希](https://github.com/OpenZeppelin/cairo-contracts/discussions/24)的更多信息。有关多调用的设计选择和实现的更多信息，请阅读[账户多调用应该如何工作](https://github.com/OpenZeppelin/cairo-contracts/discussions/27)讨论。
...
* nonce是防止交易重放的唯一标识符。当前的实现要求nonce是递增的

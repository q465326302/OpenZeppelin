...
OpenZeppelin的Governor合约与Compound的GovernorAlpha或Bravo在接口兼容性上并不相同。尽管事件完全兼容，但提案生命周期函数（如创建、执行等）具有不同的签名，这些签名旨在优化存储使用。同样，GovernorAlpha和Bravo的其他函数也无法使用。通过继承GovernorStorage模块，可以选择加入一些类似Bravo的行为。这个模块提供了提案的可枚举性和只需要提案id的队列、执行和取消函数的替代版本。这个模块减少了一些操作所需的calldata，但增加了存储占用。对于一些L2链来说，这可能是一个不错的权衡。它还为无索引器前端提供了基本元素。

请注意，即便使用了这个模块，与Compound的GovernorBravo的一个重要区别是`proposalId`的计算方式。Governor使用提案参数的哈希值来通过事件索引保持其数据的链下存储，而原始的Bravo实现使用的是顺序的`proposalId`。

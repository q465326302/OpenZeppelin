...
> IMPORTANT
此合约不包括公开的暂停和取消暂停功能。除了继承此合约，您还必须定义这两个功能，调用*Pausable._pause*和*Pausable._unpause*内部功能，并使用适当的访问控制，例如使用*AccessControl*或*Ownable*。如果不这样做，将使合约的暂停机制无法达到，从而无法使用。

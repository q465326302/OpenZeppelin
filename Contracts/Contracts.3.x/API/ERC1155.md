...
* 如果调用者不是发送方，必须通过*setApprovalForAll*方法获得发送方的授权以使用其代币。
...
* 如果to引用一个智能合约，则它必须实现*IERC1155Receiver.onERC1155BatchReceived*并返回接受的magic value。
...
* 如果to是一个智能合约，则它必须实现*IERC1155Receiver.onERC1155BatchReceived*并返回接受的magic value。

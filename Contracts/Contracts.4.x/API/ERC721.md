...
#### _burn(uint256 tokenId)
内部#
请参见 *ERC721._burn*。此重写还会检查是否为该令牌设置了特定于令牌的URI，如果是，则从存储映射中删除令牌URI。
...
#### _burn(uint256 tokenId)
内部# 
请查看*ERC721._burn*。此重写还清除令牌的版税信息。
...
#### _baseURI() → string
内部#
计算*tokenURI*的基本URI。如果设置，则每个令牌的结果URI将是baseURI和tokenId的连接。默认为空，可以在子合约中重写。

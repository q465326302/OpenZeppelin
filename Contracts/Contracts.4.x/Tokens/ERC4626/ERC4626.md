# ERC4626
*ERC4626*是*ERC20*的扩展，提出了一种标准接口来管理代币保险库。这个标准接口可以被不同的合约使用（包括借贷市场、聚合器和固有利息代币），这带来了许多细微之处。了解这些潜在问题对于实现合规和可组合的代币保险库至关重要。

我们提供了一个基本的ERC4626实现，其中包括一个简单的保险库。这个合约设计的方式使得开发人员可以轻松地重新配置保险库的行为，只需进行最少的重写，同时保持合规。在本指南中，我们将讨论一些影响ERC4626的安全考虑因素。我们还将讨论保险库的常见自定义。

## 安全问题：通货膨胀攻击

### 可视化保险库
交换ERC4626保险库中存入的资产，用户会得到股份。这些股份可以稍后被销毁，以赎回相应的基础资产。用户获得的股份数量取决于他们存入的资产数量和保险库的汇率。这个汇率由保险库持有的当前流动性定义。
...
当在对数-对数刻度上绘制时，汇率的定义方式类似，但外观不同（因为点（0,0）是无限远的）。汇率由具有不同偏移量的“对角线”表示。
...
### 攻击
存入代币时，用户获得的股份数量会向下舍入。这种舍入会从用户处取走价值，转移到保险库中（即所有当前股份持有者的利益）。由于涉及的金额很少，这种舍入通常是可以忽略的。如果您存入1e9股份的代币，舍入将最多使您损失0.0000001%的存款。但是，如果您存入价值为10股份的代币，则可能损失10%的存款。更糟糕的是，如果您存入少于1股份的代币，则您将获得0股份，基本上是一种捐赠。

对于一定数量的资产，您获得的股份越多，您就越安全。如果您想将损失限制在最多1％以下，您需要获得至少100股份。
![erc4626-4.png](img/erc4626-4.png)
存入资产

在图中，我们可以看到对于500个资产的存款，我们获得的股份数量和相应的舍入损失取决于汇率。如果汇率是橙色曲线的汇率，则我们获得的股份少于1个，因此我们损失100％的存款。但是，如果汇率是绿色曲线的汇率，则我们获得5000个股份，将我们的舍入损失限制在最多0.02％。
![erc4626-5.png](img/erc4626-5.png)
铸造股份

对称地，如果我们专注于将损失限制在最多0.5％，则我们需要获得至少200股份。对于绿色汇率，这仅需要20个代币，但对于橙色汇率，这需要200000个代币。
...
攻击者通常会等待用户进行第一次存款，然后使用上述攻击抢先进行操作。风险很低，操纵保险库所需的“捐赠”大小相当于被攻击的存款大小。

...
#### safeTransferFrom(address from, address to, uint256 tokenId)
公开#
安全地将给定的代币ID的所有权转移到另一个地址。如果目标地址是一个合约，它必须实现*IERC721Receiver.onERC721Received*函数，在安全转移时调用，并返回magic valuebytes4(keccak256("onERC721Received(address,address,uint256,bytes)"))；否则，转移将被撤销。要求msg.sender是所有者、已批准者或操作员。

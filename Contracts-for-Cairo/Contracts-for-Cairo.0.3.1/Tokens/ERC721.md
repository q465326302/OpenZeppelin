...
* tokenURI返回查询的代币URI的felt表示。然而，EIP721标准规定返回值应为字符串类型。如果未设置代币的URI，则返回值为0。请注意，URI不能超过31个字符。请参阅[解释ERC721 URI](#解释erc721-uri)。
